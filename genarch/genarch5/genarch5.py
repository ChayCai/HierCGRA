...
if __name__ == "__main__": 
    import genarch.genarch5.units as units
    ...
    for pe0 in["BlockPE" + str(idx) for idx in range(16)]:
        for pe1 in ["BlockPE" + str(idx) for idx in range(16)]:
            if(pe0 == pe1):
                continue
            core._appendConnect(pe0 + ".out0", pe1 + ".in0")
            core._appendConnect(pe0 + ".out0", pe1 + ".in1")
    for pe0 in["BlockPE" + str(idx) for idx in range(16)]:
        for pe1 in ["BlockIO" + str(idx) for idx in range(8)]:
            core._appendConnect(pe0 + ".out0", pe1 + ".in0")
            core._appendConnect(pe0 + ".out0", pe1 + ".in1")
            core._appendConnect(pe1 + ".out0", pe0 + ".in1")
    for idx in range(0, 8):
        core._appendConnect("BlockIO" + str(idx) + ".out0", "out" + str(idx))
        core._appendConnect("in" + str(idx), "BlockIO" + str(idx) + ".in0")
    for idx in range(0, 4):
        typeSwitch = core._createSwitch(1, 2)
        core._info["switch"]["SW_PE_MEM_"+str(idx)] = typeSwitch
        core._info["connection"].extend(core._connectSwitch(["BlockPE" + str(idx * 4) + ".out0"], ["BlockMEM" + str(idx) + ".in0", "BlockMEM" + str(idx) + ".in1"], "SW_PE_MEM_"+str(idx)))
        ...
        typeSwitch = core._createSwitch(1, 2)
        core._info["switch"]["SW_PE_MEM_"+str(4 + idx)] = typeSwitch
        core._info["connection"].extend(core._connectSwitch(["BlockPE" + str(idx * 4 + 3) + ".out0"], ["BlockMEM" + str(4 + idx) + ".in0", "BlockMEM" + str(4 + idx) + ".in1"], "SW_PE_MEM_"+str(4 + idx)))
    ...
    for name in names: 
        ...
        with open(f"./arch/arch5/{name}.json", "w") as fout: 
            fout.write(json.dumps(top, indent=4))
        arch = Arch(f"./arch/arch5/{name}.json")
        with open(f"./arch/arch5/{name}_RRG.txt", "w") as fout: 
            fout.write(arch.rrg())
        with open(f"./arch/arch5/{name}_FUs.txt", "w") as fout: 
            fout.write(arch.fus())

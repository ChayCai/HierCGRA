...
if __name__ == "__main__": 
    import genarch.genarch4.units as units
    ...
    for name in names: 
        ...
        with open(f"./arch/arch4/{name}.json", "w") as fout: 
            fout.write(json.dumps(top, indent=4))
        arch = Arch(f"./arch/arch4/{name}.json")
        with open(f"./arch/arch4/{name}_RRG.txt", "w") as fout: 
            fout.write(arch.rrg())
        with open(f"./arch/arch4/{name}_FUs.txt", "w") as fout: 
            fout.write(arch.fus())
